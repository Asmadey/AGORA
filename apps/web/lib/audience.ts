 ...
 *
 * ─── Откуда числа ────────────────────────────────────────────────────────
 * Было: умолчание 20, границы 1–100 — 20 стояло по приёмке #9, когда отчёт
 * ещё не требовал разрезов. Заказчик требует колонку «14–35 лет» у КАЖДОГО
 * показателя, а эта группа составляет 74 записи корпуса из 165, то есть
 * 44.8 %, и генератор сэмплирует возрастные группы по корпусным долям.
 * Значит в срез попадает около 0.45 набора:
 *
 *     набор  20 → в срезе  9, один человек 11 п.п.
 *     набор  40 → в срезе 18, один человек 5.6 п.п.
 *     набор 100 → в срезе 45, один человек 2.2 п.п.
 *     набор 200 → в срезе 90, один человек 1.1 п.п.
 *
 * Полевые волны самого заказчика — от 28 до 117 человек.
 *
 * Решение владельца 17.09.2026: минимум 40, потолок 200, умолчание 100.
 * Умолчание выше минимума намеренно: сорок — это «можно», а сто — «доли в
 * срезе имеют смысл». Прогон ниже сотни законен, но колонка «14–35» в нём
 * выглядит такой же точной, как у заказчика, будучи вдвое грубее.
 */
export const DEFAULT_AUDIENCE_SIZE = 100;
export const AUDIENCE_SIZE_BOUNDS = { min: 40, max: 200 } as const;
