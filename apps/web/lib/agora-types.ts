...
export interface SurveyRow {
  ...
  themeId?: string;
  /**
   * Варианты ответа ИМЕННО этой строки. Без них берутся общие у вопроса — так
   * устроен вопрос 9 заказчика, где один список повторяется на сорока трёх
   * подтемах. У своей матрицы оператора вопросы внутри темы разные, и список
   * лежит у каждого.
   */
  options?: SurveyOption[];
  /** Сколько вариантов персона выбирает в этой строке. Без него — ровно один. */
  maxChoices?: number;
}
