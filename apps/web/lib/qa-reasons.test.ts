...
/**
 * Источник вердикта доезжает до карточки и решает, считается ли ответ.
 *
 * Решение владельца 17.09.2026: QA-агент информирует, а не блокирует. Граница
 * проведена по источнику: детерминированные правила (`qa/checks.py`) ловят
 * объективный брак — балл вне шкалы, таймкод за пределами ролика — и такой
 * ответ в среднее не положишь. Вердикт судьи субъективен, ошибается в трёх
 * случаях из четырёх (`0ba9e30`) и потому только помечает.
 *
 * Питон уже так и делает (`analytics/aggregate.py`, `GATING_SOURCES`). Здесь
 * проверяется, что поле вообще ДОЕЗЖАЕТ до вида: `report-view` переносил
 * `kind`, `confidence` и `reasons`, а `source` терял — и веб не мог отличить
 * правило от судьи. Тот же класс дефекта, что `?? str(f.verdict)` выше.
 */
test("источник вердикта QA переносится в вид карточки", () => {
  const view = answerWith([
    { kind: "grounding", source: "rule", verdict: "regenerate", reasons: ["таймкод"] },
    { kind: "consistency", source: "judge", verdict: "regenerate", reasons: ["смысл"] },
  ]);
  assert.deepEqual(
    view.qaFlags.map((f) => f.source),
    ["rule", "judge"],
    "источник вердикта потерян — веб не отличит объективный брак от мнения судьи",
  );
});

test("флаг QA без source читается как правило", () => {
  // Отчёты, снятые до появления поля, его не содержат. Умолчание
  // консервативное, как и в питоне: ведём себя как раньше.
  const view = answerWith([
    { kind: "consistency", verdict: "regenerate", reasons: ["…"] },
  ]);
  assert.equal(view.qaFlags[0].source, "rule");
});
