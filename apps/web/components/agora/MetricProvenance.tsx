    ...
    <details open={open} className="mt-2 border-t border-hairline pt-2">
      ...
      {mismatch && (
        <p className="mt-1 text-xs text-amber-400">
          По этим {rows.length} ответам получается {computed}, а в шапке {reported}:{" "}
          {rows.length + excluded + silent < total
            ? `на экране первая страница карточек, а число посчитано по всем ${total} ответам прогона.`
            : "число в шапке посчитано при сборке отчёта, а здесь пересчитано сейчас. " +
              "У отчётов, снятых до 17.09.2026, из агрегата выбывали все помеченные " +
              "проверкой ответы, а теперь — только нарушившие правило."}
        </p>
      )}
