...
## 6. Обновление уже развёрнутого приложения

**Слияние в `main` ничего не разворачивает.** Код лежит в git, а работает образ,
собранный на сервере. Пока образ не пересобран, по адресу открывается версия
интерфейса на момент последней сборки — и выглядит это так, будто изменения не
сделаны.

Всё выполняется **на сервере**, по SSH. `/root/AGORA` — путь на VPS, а не на
рабочей машине:
...
git pull
docker compose -f infra/docker-compose.yml -f infra/docker-compose.managed.yml \
  --env-file .env.local up -d --build web worker
```

`--build` обязателен. Без него `up -d` увидит, что контейнеры запущены с тем же
описанием, и не тронет их: команда завершится успешно, ничего не изменив.

**Миграции — до сборки, а не после.** `migrate.sh` читает `.env.local` и требует
`POSTGRES_ADMIN_URL`, `AGORA_APP_PASSWORD`, `AGORA_SHARE_PASSWORD`. Запускается
там, где эти переменные есть и откуда доступен managed-инстанс, — то есть на
сервере:

```bash
cd /root/AGORA && set -a; . ./.env.local; set +a && ./infra/postgres/migrate.sh
```

Порядок «сначала схема, потом приложение» существен: новый код, встретивший
старую схему, падает на первом же запросе, а старый код на новой схеме обычно
работает. Миграции идемпотентны — повторный запуск безопасен.

Проверка после обновления:

```bash
curl -fsS https://agora.185-154-194-125.sslip.io/api/health && echo OK
```
