...
#
# Стоит ВЫШЕ `COPY pyproject.toml` намеренно: torch ставится по имени, из своего
# индекса, и списка зависимостей не читает. Копирование файла инвалидирует все
# слои ниже, так что при обратном порядке правка одной строки в зависимостях
# тянула torch заново. Замерено 17.09.2026: полоса сервера до PyPI — от 47 до
# 350 кБ/с, и такая пересборка занимает часы вместо минут.
...
# Запасного пути у этой строки нет намеренно.
#
# Здесь стояло `|| pip install --upgrade pip`. Правая часть выполняется, когда
# левая упала, и завершается успехом всегда — pip уже стоит. То есть слой
# закрывался кодом 0 при НЕ установленных зависимостях, а BuildKit кэшировал его
# как удачный.
#
# 17.09.2026 сеть сервера оборвала загрузку на `pydantic>=2.9` («from versions:
# none» при живом индексе). Слой закрылся успехом, и сборка упала тремя шагами
# ниже — на утверждении про opencv, с текстом про потолок `scenedetect<0.7`.
# К причине этот диагноз отношения не имел, а повторный `deploy.sh` брал
# сломанный слой из кэша и падал там же за 90 секунд. Пересборка перестала
# чинить сборку.
#
# Сеть у сервера рвущаяся — это лечится повторами внутри pip, а не тем, что
# отказ переписывается в успех.
RUN pip install --retries 10 --timeout 120 -e ".[dev]"
