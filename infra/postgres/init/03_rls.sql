...
-- Публичной странице нужна сама запись ссылки: по ней она узнаёт `scope` и —
-- главное — ОТЛИЧАЕТ мёртвую ссылку от несуществующей.
--
-- ─── Почему здесь НЕТ условий жизни ──────────────────────────────────────
-- Они тут были и делали ветки «отозвана» и «просрочена» недостижимыми: для
-- мёртвого токена запрос возвращал ноль строк, и код не мог сказать ничего,
-- кроме «такой ссылки нет». Человек с устаревшей ссылкой не отличал «срок
-- вышел» от «я ошибся в адресе» и писал владельцу не о том. Задача #29 требует
-- 410 на истёкшую и отозванную, 404 на неугаданную.
--
-- Снять их можно ИМЕННО ЗДЕСЬ, потому что отзыв и срок продублированы в каждой
-- политике, которая открывает ДАННЫЕ: `tasks_public_share_read` (миграция 39)
-- проверяет их своим EXISTS, `reports_public_share_read` ниже — тоже,
-- `report_share_views_public_insert` не даёт мёртвой ссылке писать в аудит-лог.
-- Сама строка `report_shares` данными отчёта не является: в ней сроки, область
-- и `task_id`, то есть состояние ссылки. Получит его только тот, кто предъявил
-- точный `token_hash`, — а значит уже держит настоящий адрес.
--
-- ─── Почему правка здесь, а не только в миграции 52 ───────────────────────
-- `migrate.sh` переприменяет 01/02/03 при каждом запуске. Политика, изменённая
-- поздней миграцией, но не изменённая здесь, молча вернулась бы к старому виду
-- при первом же прогоне скрипта — и 410 перестал бы работать, не оставив следа
-- в диффе. §5 CLAUDE.md описывает ровно этот способ потерять правку; так
-- однажды исчез FORCE ROW LEVEL SECURITY. Миграция 52 остаётся: она догоняет
-- базы, где 03_rls.sql уже применён и заново не запускается.
DROP POLICY IF EXISTS report_shares_public_read ON report_shares;
CREATE POLICY report_shares_public_read ON report_shares
  FOR SELECT TO agora_share
  USING (token_hash = app.current_share_token_hash());
