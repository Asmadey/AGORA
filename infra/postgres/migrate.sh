...
# ─── Всё, что пронумеровано выше 10 ──────────────────────────────────────────
#
# Циклом, а не отдельной веткой на файл. Ветки выше оставлены как есть: у каждой
# свой warn, объясняющий, чем грозит отсутствие именно этого файла, и заменять их
# безымянным циклом значило бы потерять эти объяснения.
#
# Цикл появился после того, как миграции 11, 12 и 13 пролежали в репозитории
# неприменёнными: скрипт про них не знал, а «Итог» напечатал OK. Отказ такого
# рода не виден ничем — код обновлён, схема нет, а расходятся они молча. Ровно
# так однажды исчез FORCE ROW LEVEL SECURITY (§5 CLAUDE.md).
#
# Порядок — числовой (`sort -V`), а не лексикографический: иначе 10 идёт перед 9.
for f in $(ls "$INIT" | grep -E '^[0-9]+_.*\.sql$' | sort -V); do
  num="${f%%_*}"
  [ "$((10#$num))" -le 10 ] && continue
  echo "   применяю $f"
  "${PSQL[@]}" -f "$INIT/$f" >/dev/null
  ok "$f"
done

# Страховка от следующей формы той же ошибки: .sql без номера в имени. Такой
# файл не подходит ни под одну ветку выше, ни под цикл, и остался бы
# неприменённым так же тихо. Здесь die, а не warn: неприменённая миграция — это
# разошедшиеся код и схема, и продолжать по ним нельзя.
for f in $(ls "$INIT" | grep -E '\.sql$' | grep -vE '^[0-9]+_' || true); do
  die "в $INIT лежит $f без номера в имени. Миграции нумерованные и только вперёд (§5), и этот файл не применён."
done
