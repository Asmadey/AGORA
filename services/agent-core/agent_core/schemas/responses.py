...
#: Значения подобраны замером на боевом запросе, а не на глаз. Ответ персоны на
#: пакете из двадцати сцен уложился в 1331–1783 токена в зависимости от прогона,
#: и потолок 2000 его ОБРЕЗАЛ: `finish_reason=length`, JSON не дописан. Три
#: сквозных прогона подряд упали именно так — двенадцать ответов из двенадцати.
#:
#: Поэтому запас двукратный: расход плавает от прогона к прогону, а цена
#: заниженного потолка — не «ответ покороче», а полностью потерянный ответ.
MAX_TOKENS = {
    "frame_analysis": 2000,
    "respondent": 4000,
    "judge": 1500,
