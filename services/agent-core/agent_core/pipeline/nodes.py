...
def _personas_for_segments(
    ...
    """
    Персоны для анкеты и посегментного разреза.

    Ответы нового прогона несут срез в поле `segment`, но анкете всё равно нужен
    реестр персон: он задаёт знаменатель общего охвата и среза 14-35. Сам
    посегментный разрез при этом по-прежнему предпочитает срез из ответа, а
    реестр использует только как запасной источник.

    Отказ чтения не роняет отчёт, а дописывается в `degraded`. Без разреза отчёт
    беднее, без охвата анкеты не будет и знаменателя её показателей; падение здесь
    стоило бы транскрипции, разбора кадров и всех ответов персон, которые уже
    оплачены.
    """
    try:
        personas = _load_personas(state)
        ...
        degraded.append(
            ...
            f"посегментного разреза и среза 14-35 с знаменателем анкеты в отчёте не будет"
        )
        return []
...
def qa(state: PipelineState) -> dict[str, Any]:
    ...
    update: dict[str, Any] = {
        ...
        #
        # Число переспрошенных приписывается ЗДЕСЬ, а не внутри summary():
        # переспрос идёт вокруг проверки, а не внутри неё, и QaOutcome о нём не
        # знает. Без этой строки `qa_requestioned` оставался в состоянии и до
        # отчёта не доезжал — то есть механизм работал, а экран продолжал
        # утверждать, что переспроса в системе нет.
        "qa_summary": {**outcome.summary(), "requestioned": requestioned},
    }
...
def _requestion_flagged(
    ...
    """
    Переспрашивает только требующие этого персоны и проверяет их ответы заново.
    ...
    # Информирующие правила остаются в `qa_flags` для оператора, но не должны
    # превращать вкусовой сигнал в оплаченный повторный вызов модели.
    flagged = outcome.requestionable
    ...
    if rq.cap_exhausted(all_targets, cap=cap):
        ...
        # «Остались помеченными», а не «исключёнными»: с 17.09.2026 из агрегата
        # выбывает только нарушивший детерминированное правило, а вердикт судьи
        # помечает карточку и не блокирует (`analytics/aggregate.py`,
        # `GATING_SOURCES`). Прежняя формулировка утверждала бы про
        # непереспрошенные ответы то, чего с ними не происходит.
        degraded.append(
            f"qa: потолок переспроса исчерпан — переспрошено {len(targets)} "
            f"ответов из {len(all_targets)} помеченных; остальные "
            f"{len(all_targets) - len(targets)} остались с пометкой (из агрегата "
