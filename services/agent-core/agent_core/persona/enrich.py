...
#: Прежняя редакция утверждала: «по ним считается persona_grounding». Для
#: `demographics` это верно — метрика сверяет age_group, geo и gender. Для
#: `values_and_beliefs` это было НЕВЕРНО: ценностей `persona_grounding` не
#: касается вовсе.
#:
#: Строка стоила месяцев. Каждый, кто открывал файл, видел «ценности
#: заземлены» и шёл дальше — а в это время генератор назначал три ценности
#: вместо пяти, ставил один и тот же топ-2 всем персонам набора и брал
#: значения из топ-15 корпуса вместо канонического перечня. Нашлось это
#: глазами владельца, а не проверкой.
#:
#: Комментарий, обещающий гарантию, которой нет, опаснее отсутствующего: на
#: него полагаются вместо того, чтобы завести проверку.
#:
#: ─── Как сейчас ──────────────────────────────────────────────────────────
#: `demographics` и баллы держит `persona_grounding` (evals/check.py).
#: `values_and_beliefs` держат `tests/test_persona_values.py` (блокируют
#: слияние) и справочная метрика `values_grounding` (порядок всех семнадцати
#: ценностей против корпуса).
#:
#: Остальные поля списка не покрыты НИЧЕМ, и это записано здесь прямо, чтобы
#: следующий читатель не принял перечисление за гарантию. Расширяя модуль,
#: объясните, почему новое поле не ломает то, что действительно проверяется.
GROUNDED_FIELDS = (
    "demographics",
    "big_five",
    "values_and_beliefs",
