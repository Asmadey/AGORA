...
def _load_traditional_values() -> tuple[str, ...]:
    """
    Канонический перечень из `data/values/traditional_values.json`.

    Отсутствие файла — отказ, а не откат на список в коде. Запасной список
    означал бы два источника правды, которые разойдутся молча: генерация пошла
    бы по старому перечню, а тесты и карточка — по новому.
    """
    if _VALUES_PATH is None:
        raise FileNotFoundError(
            "не найден data/values/traditional_values.json — из чего собирать "
            "ценности персоны, неизвестно. В образе воркера каталог data/ "
            "копируется рядом с исходниками (infra/worker.Dockerfile)"
        )
    doc = json.loads(_VALUES_PATH.read_text("utf-8"))
    values = doc.get("values") or []
    if len(values) != 17:
        raise ValueError(
            f"в справочнике {len(values)} ценностей вместо 17 — перечень Указа 809 "
            f"неполон, и выборка молча пошла бы по укороченному списку"
        )
    return tuple(values)


#: Семнадцать традиционных ценностей. Пул выборки — ровно они.
#:
#: Здесь стоял `VCIOM_TO_DNA` — словарь, который выглядел белым списком и им не
#: был: `VCIOM_TO_DNA.get(v, v)` пропускает незнакомое значение насквозь. Пул
#: при этом строился не из него, а из `most_common(15)` по корпусу, поэтому в
#: персону попадало «Неравенство, разделение людей в соответствии с их
#: способностями» (3.0 % корпуса) — прямая противоположность ценности, а также
#: служебные ответы анкеты вроде «Затрудняюсь ответить».
#:
...
def even_sample(items: list[str], n: int) -> list[str]:
    """
    n реплик из пула равномерным шагом, а не первые n.

    В порядке корпуса первые двадцать — это почти один материал: 17 из 20
    приходились на «Константинополь». Шаг сохраняет детерминизм и даёт речь со
    всего корпуса.

    Живёт здесь, рядом с самим пулом, а не по месту вызова: тот же отбор нужен
    судье связности (`persona/tasks.py`) и опросу персон (`pipeline/nodes.py`),
    и три копии одного шага разошлись бы молча.
    """
    if not items:
        return []
...
#:
#: Снимается при сборке пула, потому что пул уходит судье связности как ОБРАЗЕЦ
#: речи корпуса (`persona.validate.md`, критерий «досочинённый факт»). Ярлык кладёт
#: перед судьёй чужой пол и чужое имя: реплика «Женщина: …» в образце для персоны
#: мужского пола читается как противоречие атрибуту.
#:
#: Замер на корпусе 16.09.2026: в `focus_group_verbatims` ярлык стоит у ВСЕХ 660
#: реплик и сводится к шести вариантам («Женщина» 376, «Мужчина» 207, «Сергей» 51,
#: «Оксана» 23, «Камилла» 2, «Полина» 1). В анкетных вербатимах его нет ни у одной
#: из 441 — поэтому чистка применяется только к стенограмме. Общая регулярка
#: срезала бы у анкетного ответа «Вывод:» или «Итог:» вместе со смыслом.
_SPEAKER_LABEL = re.compile(r"^\s*[А-ЯЁ][а-яё]*\s*:\s*")
    ...
    def from_corpus(cls, records: list[dict[str, Any]]) -> CorpusDistribution:
        ...
        for r in records:
            ...
            for fv in r.get("focus_group_verbatims", []):
                # Ярлык снимается ДО проверки длины: иначе «Женщина: да.» проходит
                # порог за счёт служебного префикса, а не за счёт содержания.
                line = _SPEAKER_LABEL.sub("", fv).strip()
                if len(line) > 20:
                    verbatims.append(line)
