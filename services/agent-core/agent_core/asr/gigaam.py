...
─── Установка ────────────────────────────────────────────────────────────────
Обычной зависимостью в `pyproject.toml`, по коммиту. Первая редакция ставила
пакет в Dockerfile руками с `--no-deps`, опасаясь, что его пин
`onnxruntime==1.23.*` понизит 1.29 «под sherpa-onnx». Опасение оказалось
выдуманным: sherpa-onnx тянет собственный `sherpa-onnx-core` и питоновский
onnxruntime не использует вовсе, а единственный, кому тот нужен, —
faster-whisper, и он принимает `<2,>=1.14`.

Цена ошибки была не в лишней осторожности, а в том, что зависимость оказалась
незаявленной: образ и `pyproject` разошлись бы молча — воркер работает, а CI,
который собирает окружение из pyproject, получает другое. Поймал это
