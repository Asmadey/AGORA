...
#: Длина куска.
#:
#: ─── Первый замер: память и скорость ──────────────────────────────────────
#: Внимание квадратично, поэтому короткий кусок дешевле в пересчёте на секунду
#: звука. Замер 18.08.2026 на дорожке 15 минут:
#:
#:      60 с → 0,103 с/с   1,2 ГБ
#:     120 с → 0,138       1,7 ГБ
#:     400 с → 0,240       4,6 ГБ
#:
#: По нему была выбрана минута. Это оказалось неверно, и вот почему.
#:
#: ─── Второй замер: КАЧЕСТВО, а его никто не мерил ─────────────────────────
#: Владелец сообщил, что на участке с явной речью в отчёте стоит «тишина».
#: Прогон № 0051 (фильм 50 минут): 43 реплики, покрытие речью 59 %, а в одном
#: месте английская бессмыслица «I come so little» поверх русского звука.
#:
#: Замер 19.08.2026, пять минут разговорной части того же фильма, одна и та же
#: дорожка, разная длина куска:
#:
#:      60 с → 107 слов,  4 сегм., 36,8 с
#:      30 с → 127 слов,  7 сегм., 30,0 с
#:      20 с → 134 слова, 8 сегм., 29,4 с
#:      15 с → 145 слов, 11 сегм., 30,3 с   ← выбрано
#:      10 с → 136 слов, 16 сегм., 30,2 с
#:
#: Пятнадцать секунд дают на 36 % больше слов, чем минута, И считаются быстрее.
#: Причина в устройстве модели: parakeet — трансдьюсер, он декодирует кусок как
#: одно высказывание. На минуте киношного звука с музыкой и паузами он
#: схлопывается и отдаёт одну короткую фразу на весь кусок — или пустоту.
#: Отдельный случай: 900–960 с при куске в минуту дают НОЛЬ слов, при 15
#: секундах — восемь.
#:
#: Ниже пятнадцати качество снова падает (136 слов на десяти): куски начинают
#: рвать фразы чаще, чем помогают.
#:
#: Урок, который стоит записать: первый замер мерил стоимость и молчал о
#: качестве, а выбор был сделан по нему одному. Быстрый и дешёвый разбор,
#: теряющий 40 % речи, — это не оптимизация.
CHUNK_SEC = 15.0
...
#: полутора секунд находится почти всегда, а когда не находится — режем по месту,
