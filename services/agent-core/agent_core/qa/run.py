...
Из того же порядка следует, что ответ, забракованный гейтящим правилом, судье
не отправляется ВООБЩЕ, а не только по забракованной проверке. Он уже помечен
на перегенерацию, и второе суждение о нём ничего не меняет - только стоит денег.
...
@dataclass
class QaOutcome:
    """Итог проверки с отдельными списками видимости и переспроса."""
    ...
    def flagged(self) -> list[dict[str, Any]]:
        """Все заметные оператору отказы, включая информирующие флаги."""
        return [v for v in self.verdicts if v.get("verdict") == "regenerate"]

    @property
    def requestionable(self) -> list[dict[str, Any]]:
        """Отказы, для которых допустим повторный вызов модели."""
        return [
            verdict for verdict in self.flagged
            if verdict.get("source") != INFORMING_RULE_SOURCE
        ]
    ...
    def summary(self) -> dict[str, Any]:
        ...
        Наружу из прогона уходят все `flagged`, включая информирующие флаги.
        Оператор видит по ним сигналы, а отдельный `requestionable` определяет,
        что требует переспроса. По `flagged` видно, сколько ответов исключено,
        и не видно ничего больше: ни по каким видам проверки, ни кем забраковано
        (правило или судья), ни сколько вердиктов ушло на эскалацию. Полный список
        вердиктов живёт в `qa_report.json` в рабочем каталоге прогона, а он
        внутри контейнера и умирает вместе с ним.

        ─── Про переспрос ──────────────────────────────────────────────────
        Забракованный ответ переспрашивается — с 19.08.2026, ровно один круг и
        в пределах потолка из настроек команды. Здесь этого поля всё равно нет,
        и причина другая, чем была: переспрос идёт ВОКРУГ проверки, в
        `pipeline/nodes.py`, потому что ему нужны разом ответы, вердикты и
        вторая проверка тех же ответов. QaOutcome о нём не знает и знать не
        должен — иначе проверка начнёт зависеть от того, что делают с её
        результатом.

        Число переспрошенных приписывается к этой сводке снаружи, там же, где
        считается. До 28.08.2026 оно не приписывалось никуда, и экран утверждал,
        что переспроса в системе нет, — механизм работал, а отчёт о нём молчал.
        """
