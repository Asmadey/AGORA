...
class PersonaAttemptsConfig:
    ...
    43 прошли проверку с первой попытки, 14 со второй, 13 с третьей, четверо не
    прошли вовсе. Пересоздание спасло 23 из 27 — механизм исправен.

    Дорог он другим: 220 вызовов модели вместо 140, то есть 57 % накладных, и
    увидеть это можно было только запросом к базе. Число попыток при этом было
    прибито константой `MAX_ATTEMPTS` в `persona/validate.py`, тогда как у
    переспроса ОТВЕТОВ настройка есть. Два похожих механизма с непохожей
    судьбой; здесь асимметрия снимается.

    Сама по себе ручка ничего не улучшает, и это стоит помнить, крутя её: доля
    успеха на попытку держится около 50–70 %, четвёртая попытка спасла бы 2–3
    персоны из четырёх ценой четырёх полных циклов. Рычаг — первая попытка.

    ─── Почему считаем попытки, а не пересоздания ─────────────────────────
    Единица — «проверить и оставить как есть», без единой замены. Ноль означал
    бы «не проверять вовсе», а это другое решение: тогда в набор попадают
    персоны, про которых неизвестно ничего, и отличить их от проверенных
    нельзя. Такую ручку надо заводить отдельно и обсуждать отдельно.

    ─── Почему из снимка ──────────────────────────────────────────────────
    По той же причине, что температуры и потолок переспроса: пока задача стоит
    в очереди, настройку можно сменить, и тогда часть персон пересоздана по
    одному правилу, часть по другому — внутри одного набора, который потом
    читают как целое.
    """

    attempts: int = 3

    #: Верхняя граница. Каждая попытка — полный цикл: генерация, обогащение,
    #: проверка, то есть два вызова модели на персону. При доле успеха 50–70 %
    #: на попытку после пятой спасать уже почти нечего — дальше платят за воздух.
    MAX: ClassVar[int] = 5

    @classmethod
    def defaults(cls) -> PersonaAttemptsConfig:
        return cls()

    @classmethod
    def for_task(cls, settings_snapshot: dict[str, Any] | None) -> PersonaAttemptsConfig:
        raw = (settings_snapshot or {}).get("personaAttempts")
        if raw is None:
            return cls()
        if isinstance(raw, bool) or not isinstance(raw, (int, float, str)):
            raise ValueError(f"число попыток не число: {raw!r}")
        try:
            attempts = int(raw)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"число попыток не число: {raw!r}") from exc
        if not 1 <= attempts <= cls.MAX:
            # Отвергаем, а не обрезаем: обрезка означала бы, что человек видит в
            # настройках одно, а прогон исполняет другое.
            raise ValueError(f"число попыток {attempts} вне диапазона 1..{cls.MAX}")
...
class RequestionConfig:
    ...
    #: Сорок, а не пятнадцать: аудитория по умолчанию выросла с двадцати до ста
    #: (17.09.2026), и прежний потолок стал пятнадцатью процентами прогона —
    #: то есть срабатывал бы всегда. Подробнее — в `respondent/requestion.py`.
    cap: int = 40
