...
─── История правила опоры и решение его снять ───────────────────────────────
Изначально приёмка требовала, чтобы каждое утверждение синтеза имело ссылку на
таймкод или цитату. Это нельзя было оставить только пунктом промпта: промпт —
просьба, и требование, которое только просят, выполняется через раз и
незаметно. Поэтому код временно отсеивал утверждения без таймкода и кавычек, а
число отсеянных писал в `degraded`.

Владелец 18.09.2026 снял принудительный отсев: на прогоне 0094 он съел часть
связного синтеза и выглядел как поломка отчёта, хотя поломалась только
эвристика фильтра. Просьба к модели ссылаться на материал остаётся, но все
непустые утверждения теперь сохраняются. Наличие опоры переносится в
`narrative_support`, чтобы наблюдение не терялось и не смешивалось с
поломками в `degraded`.
...
def build_report(
    *,
    ...
    """Отчёт: агрегат, точки риска и полный непустой синтез модели."""
    degraded: list[str] = []
    agg = aggregate(
        answers,
        survey=survey,
    ...
    report: dict[str, Any] = {
        ...
        # осталось исключённым, по видам и по источнику вердикта. «Исключено»
        # здесь — ИТОГ, после переспроса: вердикты второго круга заменяют
        # вердикты первого, поэтому «переспрошено 8, исключено 3» значит, что
        # пять ответов вернулись годными.
        ...
        "rationales": {},
        "narrative": [],
        "narrative_support": {"total": 0, "supported": 0},
        "themes": [],
        "disagreements": [],
        "strengths": [],
        "weaknesses": [],
    ...
    report["narrative_support"] = {
        "total": len(narrative),
        "supported": sum(1 for statement in narrative if has_support(statement)),
    }
