...
#: Пакет материала: таймлайн, сцены, транскрипт. Отдельный документ, а не поле
#: отчёта: отчёт читают целиком на каждой загрузке экрана, а пакет — только
#: когда открывают таймлайн, и на пятнадцатиминутном ролике он в разы больше
#: самого отчёта.
...
def save_content_pack(
    ...
    *,
    ...
    """
    Кладёт пакет материала рядом с отчётом.

    ─── Зачем ──────────────────────────────────────────────────────────────
    Пакет собирался, уезжал в промпты персон и умирал вместе с контейнером: он
    жил только в состоянии графа и в `/tmp/agora/<task>`, а этот каталог не
    смонтирован. Экрану исследования показывать таймлайн было не из чего — при
    том что каждая его ячейка уже посчитана и оплачена.

    Сохраняется ПОЛНАЯ форма: компактная урезана до ключевых сцен ради цены
    запроса к модели, а человеку на экране нужен весь материал. Идемпотентно по
    (tenant_id, task_id) — перезапуск дописывает поверх, а не заводит второй.
    """
