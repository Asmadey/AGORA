...
#:
#: Решение владельца 17.09.2026: «задача QA-агента не блокировать появление
#: ответов, а информировать, где они ошибаются». На уровне персон так и было —
#: забракованная остаётся в наборе и отвечает. На уровне ответов был сделан
#: обратный выбор, и цена его измерена на боевом: в одном прогоне отчёт собран
#: по 15 ответам из 40, остальные 25 исключены.
#:
#: Но «информировать» нельзя распространить на всё. QA ответов — это две разные
#: вещи. Детерминированные правила (`qa/checks.py`) ловят объективный брак: балл
#: вне шкалы 1–10, таймкод за пределами ролика, пустые вербатимы, незакрытые
#: вопросы анкеты. Балл 15 по десятибалльной шкале в среднее не положишь, и это
#: не вопрос мнения.
#:
#: LLM-судья ловит субъективное и ошибается: `0ba9e30` — «три четверти
#: отбраковок QA оказались дефектом правила, а не качеством». Его вердикт
#: остаётся на карточке, виден человеку и в сводке — но ответ считается.
#:
#: `escalated` — тот же вердикт судьи, перепроверенный моделью побольше.
#: Субъективность от размера модели не исчезает, поэтому он тоже информирует.
GATING_SOURCES = frozenset({"rule"})


def _flag_keys(qa_flags: list[dict[str, Any]] | None) -> set[tuple[str, int]]:
    """Адреса ответов, выбывающих из агрегата по нарушению правил."""
    ...
    for flag in qa_flags or []:
        ...
        if persona_id is None:
            ...
            continue
        # Умолчание «rule», а не «judge»: отчёты, снятые до появления поля
        # `source`, его не содержат, и считать их субъективными значило бы
        # задним числом вернуть в агрегат ответы, которых там не было. Прежние
        # отчёты пересчитались бы иначе, чем их читали.
        if str(flag.get("source") or "rule") not in GATING_SOURCES:
            continue
