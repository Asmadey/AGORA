...
def personas(n: int) -> list[dict]:
    return [
        {"id": f"p{i}", "name": f"Персона {i}",
         # Метка в DNA — единственный способ отличить персон в system-промпте:
         # идентификатор и имя туда не подставляются, в шаблон уходит только DNA.
         #
         # Именно в `lifestyle_and_interests`, а не в `narrative`: с 18.09.2026
         # портрет персоне на опросе не показывают — он перечислял ей её же
         # ценности там, где спрашивают про ценности материала.
         "dna": {"demographics": {"age_group": "25-34"},
                 "lifestyle_and_interests": {"hobbies": [f"метка {i}"]},
                 "narrative": f"смотрит сериалы {i}"}}
        for i in range(n)
    ]
