...
def test_respondent_schema_is_given_not_invented():
    """
    Схема у ответа персоны есть, но клиент её НЕ СТРОИТ. Ему её передают.

    ─── Что здесь было раньше ───────────────────────────────────────────────
    Стояло `assert "response_format" not in source` — запрет на схему целиком,
    по замеру: двенадцать боевых персон, один промпт, температура 0.3, бок о
    бок — со схемой разобрано 8 из 12, без неё 12 из 12. Четыре ответа
    вырождались под грамматикой (пробельное залипание, счётчик внутри строки) и
    шли до потолка, не дописав JSON. Теряется не поле, а весь оплаченный ответ.

    Тест сработал ровно так, как было задумано: он покраснел, когда схему
    вернули. Поэтому он не удалён, а переписан — вместе с причиной.

    ─── Что решено и что осталось защищённым ────────────────────────────────
    Владелец решил 17.09.2026 вернуть схему в связке с правилом покрытия и
    переспросом: неполный ответ объявлен ошибкой, и одной проверкой постфактум
    её не закрыть.

    Прежняя опасность при этом не исчезла, и защита от неё — в том, что схема
    приходит ПАРАМЕТРОМ. Клиент, который строил бы её сам, нельзя было бы
    выключить на прогоне: замер против схемы понадобился однажды и понадобится
    снова, а сравнивать две ветки кода вместо двух прогонов — не замер.

    Умолчание — без схемы. Включает её тот, кто знает и анкету, и настройки
    прогона (`pipeline/nodes.py`).
    """
    ...
    assert "answer_schema" in source, "схема должна приходить параметром"
    assert "answer_json_schema(" not in source, (
        "клиент строит схему сам — выключить её на прогоне будет нечем"
    )

    client = QwenRespondentClient.__new__(QwenRespondentClient)
    assert "answer_schema: dict[str, Any] | None = None" in source, (
        "умолчание обязано быть «без схемы»"
    )
    del client
