...
# ─── 10. Проверка персоны — не обогащение ────────────────────────────────────
#
# Найдено по трассе набора из 60 персон 21.08.2026: в ней 198 наблюдений, и все
# названы `enrich-persona`. На деле обогащений там 60, ещё около сотни — вызовы
# ПРОВЕРКИ связности, и сорок — повторные обогащения пересозданных персон.
#
# Причина в том, что оба режима идут через один класс клиента, а имя наблюдения
# в нём зашито строкой. Владелец увидел «слишком много enrich» и был прав:
# половина этих вызовов делает другую работу.
#
# Цена ошибки — не косметика. По трассе нельзя ответить, на что ушли токены:
# обогащение и проверка стоят по-разному и чинятся в разных местах.

def test_persona_validation_is_named_apart_from_enrichment():
    """
    Имя наблюдения выводится из роли клиента, а не задано одной строкой.

    Проверка по исходнику: поднимать провайдера ради имени спана незачем, а
    подменённый клиент не докажет, что имя различается в двух режимах.
    """
    import ast
    from pathlib import Path

    source = Path(tracing.__file__).resolve().parent / "persona" / "enrich.py"
    tree = ast.parse(source.read_text(encoding="utf-8"))
    ...
    assert not names, (
        "имя наблюдения зашито строкой: проверка связности попадёт в трассу под "
        "именем обогащения, и разобрать, на что ушли токены, будет нельзя"
    )
