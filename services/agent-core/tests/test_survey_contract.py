...
def test_question_text_is_not_empty():
    ...
    ─── Почему проверка переписана ──────────────────────────────────────────
    17.09.2026 рендер стал многострочным: у закрытого вопроса под формулировкой
    идут его варианты, у матрицы — строки и заголовки тем. Прежняя проверка
    разбирала вывод построчно и делила по «)», то есть держалась за формат, а
    не за утверждение. Утверждение осталось прежним и проверяется прямо: у
    каждого вопроса есть свой блок, и вторая строка блока — непустая
    формулировка.
    """
    out = _render_questions(WEB_SURVEY)
    blocks = [b for b in out.split("\n\n") if b.strip()]
    assert len(blocks) == len(WEB_SURVEY), (
        f"блоков {len(blocks)}, вопросов {len(WEB_SURVEY)} — вопрос потерян или склеен"
    )
    for block in blocks:
        lines = block.splitlines()
        assert lines[0].startswith("["), f"блок не начинается с идентификатора: {lines[0]!r}"
        assert len(lines) > 1 and lines[1].strip(), (
            f"вопрос без формулировки: {lines[0]!r}"
        )
