...
def persona(idx: int, secret: str | None = None) -> dict:
    """
    Метка кладётся в `lifestyle_and_interests`, а НЕ в `narrative`.

    С 18.09.2026 портрет персоне на опросе не показывают: он перечислял ей её же
    ценности ровно там, где спрашивают про ценности материала (эхо 89 % против
    случайных 29 %, прогон 0093). Метка в `narrative` после этого не доезжает до
    промпта, и проверка изоляции стала бы зелёной на пустом месте — она бы не
    находила утечку просто потому, что не находила бы ничего.

    Проверяемое свойство не изменилось: маркер одной персоны не имеет права
    появиться в промпте другой. Сменился только носитель.
    """
    return {
        "id": f"p{idx}", "name": f"Персона {idx}",
        "dna": {
            "demographics": {"age_group": "25-34", "age": 30},
            "lifestyle_and_interests": {"hobbies": [secret or f"портрет {idx}"]},
            "narrative": f"портрет {idx}",
        },
    }
