...
def check_secret_scan():
    ...
    pat = re.compile(
        ...
        r"(?!\$)[^\s@$]{6,}@"
        # Литеральный пароль в присваивании или в теле формы — то есть ключ
        # password со строковым значением справа.
        #
        # Добавлено 04.08.2026. Прежняя редакция знала только ключи API и пароли
        # в строках подключения, поэтому два пароля учётных записей приложения,
        # лежавшие в evals/tests открытым текстом, метрику не покраснели.
        #
        # (?![$<]) обязателен: `-v app_password="$AGORA_APP_PASSWORD"` в
        # migrate.sh — это ссылка на переменную окружения, то есть как раз
        # правильный способ, и краснеть на нём нельзя. Гейт, дающий ложные
        # срабатывания, начинают игнорировать, и он перестаёт ловить настоящее.
        r"|[\"']?pass(?:word|wd)?[\"']?\s*[:=]\s*[\"'](?![$<])[^\"'\s]{6,}[\"']"
    )
    ...
    # «evals» убран из исключений 04.08.2026. Именно из-за него два пароля
    # учётных записей приложения пролежали в evals/tests открытым текстом при
    # зелёной метрике: каталог с тестами не обходился целиком. Метрика, которая
    # не смотрит туда, где пишут люди, создаёт ровно то ложное спокойствие, ради
    # борьбы с которым её и завели.
    #
    # Каталоги с данными (fixtures, grounding, artifacts) исключаются по имени и
    # потому по-прежнему не сканируются, где бы ни лежали.
    # .claude — рабочие каталоги агентов (worktrees). Это копии репозитория, а не
    # он сам: находки в них дублируют находки в дереве и указывают на файлы,
    # которых в коммите не будет.
    skip_dirs = {"node_modules", ".venv", ".git", ".next", "dist", "build", "__pycache__",
                 "fixtures", "grounding", "artifacts", ".claude"}
    ...
    for root, dirs, files in os.walk(REPO):
        ...
        for f in files:
            ...
            if not is_code and not f.endswith(doc_exts):
                continue
            # Исходник самого сканера не сканируется: он обязан содержать
            # шаблоны, которые ищет, и находит их у себя. Прежде это было скрыто
            # исключением каталога evals целиком — то есть дефект существовал
            # всегда, а проявился, только когда каталог начали обходить.
            if Path(root) / f == Path(__file__).resolve():
                continue
