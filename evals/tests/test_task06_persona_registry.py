...
# Карточка + компонент, которому она делегирует отрисовку DNA.
#
# Разбор DNA переехал из страницы в `PersonaDnaView`, потому что ровно те же
# данные показывает попап «О персоне» в отчёте, и две копии разошлись бы
# сначала подписями, потом составом полей. Тест же продолжал читать одну
# страницу и краснел на «перечислении полей», которого там просто больше нет.
#
# Проверяется по-прежнему свойство, а не место: обход структурой и канонический
# тип. Если завтра отрисовка переедет ещё раз, красным станет этот список — и
# это правильно, потому что список и есть ответ на вопрос «где смотреть».
DNA_VIEW = WEB / "components" / "agora" / "PersonaDnaView.tsx"
