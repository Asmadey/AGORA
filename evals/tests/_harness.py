...
def worker_deps_missing(*modules: str) -> str | None:
    ...
    `openai`, `faster_whisper` и `pyannote.audio` живут в образе воркера. Там
    они и нужны: ставить torch на хост ради прогона тестов значит завести
    вторую среду, которая разойдётся с первой и никем сверяться не будет.

    Без этой проверки отсутствие зависимости приезжает в вывод как дефект кода,
    и приезжает поздно — не при импорте, а при вызове: `faster_whisper`
    подтягивается внутри `transcribe()`, `openai` — внутри клиента. Поэтому
    прежние гейты не срабатывали: они проверяли импорт обёртки, а падало
    содержимое. Прогон на сервере 12.08 показал это у четырёх задач сразу
    (#15, #18, #19, #20), и ни одна претензия не была правдой.
    ...
    if not absent:
        return None
    return (
        f"нет {', '.join(absent)} — эти зависимости живут в образе воркера, а не "
        f"на хосте. Запустите тест там: ./evals/run_in_worker.sh <файл теста>"
    )
...
class ApiClient:
    ...
    def login(self, email: str, password: str) -> tuple[bool, str]:
        ...
        ─── BASE_URL обязан быть тем же https, что и AUTH_URL ─────────────────
        В production Auth.js выдаёт куку сессии с префиксом `__Secure-` и
        флагом `secure`, и по http её не поставит вообще. Вход через
        `http://127.0.0.1:3000` поэтому отвечает 200 и не оставляет куки —
        снаружи это неотличимо от неверного пароля, и сообщение ниже
        («проверьте пароль и членство») уводит искать не туда. Берите
        `BASE_URL="$AUTH_URL"`, даже когда сервер рядом.
        """
...
def drop_task(client: "ApiClient", task_id: str) -> None:
    """
    Убирает прогон, заведённый тестом.

    Поведенческие проверки создают НАСТОЯЩИЕ прогоны в среде пользователя: они
    видны в списке исследований наравне с рабочими, и отличить их можно только
    по автору. К 28.08.2026 таких накопилось десять, шесть из них упавших, и
    владелец принял их за свои.

    Отказ удаления не роняет тест — уборка проверяет не то, ради чего он
    написан, — но и НЕ ЗАМАЛЧИВАЕТСЯ. Первая редакция глотала всё подряд, и
    когда уборка не сработала, это было видно только по числу строк в базе:
    молчаливая уборка неотличима от сработавшей.
    """
    try:
