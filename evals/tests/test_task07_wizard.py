...
# ─── Исследование привязывается к проекту ────────────────────────────────────
#
# `tasks.project_id` есть в схеме с самого начала, `POST /api/tasks` принимает
# `projectId` и пишет его — а визард не отправлял это поле НИ РАЗУ. Каждое
# исследование, созданное через интерфейс, ложилось в базу с project_id = NULL:
# механизм был цел на всех уровнях, кроме того единственного, через который им
# пользуются.
#
# Такой дефект статические проверки пропускают по построению: и колонка, и
# маршрут, и запись на месте — не хватает только вызывающего. Поэтому проверка
# смотрит на визард, а не на маршрут.

wizard_src = (
    REPO / "apps" / "web" / "app" / "studies" / "new" / "page.tsx"
).read_text("utf-8")

launch_body = ""
if "await fetch(\"/api/tasks\"" in wizard_src:
    launch_body = wizard_src.split("await fetch(\"/api/tasks\"", 1)[1][:600]

check(
    "визард отправляет projectId при запуске",
    "projectId" in launch_body,
    "в теле POST /api/tasks нет projectId — исследование ляжет в базу без проекта, "
    "и раздел «Проекты» останется пустым при любом числе прогонов",
)

check(
    "на шаге «Контент» есть выбор проекта",
    "/api/projects" in wizard_src or "ProjectPicker" in wizard_src,
    "визард нигде не спрашивает проект: выбирать не из чего",
)
